...
- [Waypoint](https://developers.google.com/maps/documentation/routes/reference/rest/v2/Waypoint)
  的 `address`、`placeId` 與 `location` 是同層互斥欄位；本 Skill 只公開前兩者。
- [Intermediate waypoints](https://developers.google.com/maps/documentation/routes/intermed_waypoints)
  支援最多 25 個 `intermediates`，stopover 會形成獨立 route leg；11–25 個中途點可能使用
  較高費率。query v2 為成本可預期而限制 0–10 個，保持輸入順序且不設定
...
- [Route response](https://developers.google.com/maps/documentation/routes/understand-route-response)
  提供 route、legs、warnings、fallback 與地址解析後的 Place ID；adapter 只保留選用
  output profile 所需欄位。
