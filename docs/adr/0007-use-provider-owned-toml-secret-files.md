...
`google-routes` 使用作業系統標準使用者設定目錄中的 TOML secret file 保存與解析 Google Maps Platform API key，不再讀取 `GOOGLE_MAPS_API_KEY`。Windows 預設為 `%LOCALAPPDATA%\command1264-skills\credentials\google-routes.toml`，macOS 預設為 `~/Library/Application Support/command1264-skills/credentials/google-routes.toml`，Linux 預設為 `${XDG_CONFIG_HOME:-~/.config}/command1264-skills/credentials/google-routes.toml`。檔案格式為帶 `schema_version` 的嚴格 TOML；v2 不提供環境變數或命令列路徑 override。
